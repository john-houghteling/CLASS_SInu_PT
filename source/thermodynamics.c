...
int thermodynamics_at_z(
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    if(pba->has_idr == _TRUE_){ //AD: All _idm_dr lines are commented out
      ...
      for(n=0; n<N_coupling_idr; n++){
        pvecthermo[pth->index_th_tau_idr+n] = pth->thermodynamics_table[(pth->tt_size-1)*pth->th_size+pth->index_th_tau_idr+n]+
        (pth->thermodynamics_table[(pth->tt_size-1)*pth->th_size+pth->index_th_tau_idr+n]-pth->thermodynamics_table[(pth->tt_size-2)*pth->th_size+pth->index_th_tau_idr+n])
        *(z-pth->z_table[pth->tt_size-1])/(pth->z_table[pth->tt_size-1]-pth->z_table[pth->tt_size-2]);
      }

      /* extrapolate idm_dr visibility function (from SInu, left out by default) */
      //pvecthermo[pth->index_th_g_idm_dr] = pth->thermodynamics_table[(pth->tt_size-1)*pth->th_size+pth->index_th_g_idm_dr];

      /* extrapolate idm_dr visibility function (using eqn from C-R paper), no good */
      /*pvecthermo[pth->index_th_g_idm_dr] = pow(pba->T_idr,2)*G_eff_sq_sum*exp(-pvecthermo[pth->index_th_tau_idr])/((1.+z)*N_coupling_idr);*/
...
int thermodynamics_merge_reco_and_reio(
  ...
  /** - add  more points to start earlier in presence of interacting DM or IDR/SInu */

  if(pba->has_idm_dr == _TRUE_ || pba->has_idr == _TRUE_) pth->tt_size += ppr->thermo_Nz1_idm_dr + ppr->thermo_Nz2_idm_dr - 1;
...
int thermodynamics_output_titles(struct background * pba,
  ...
  if(pba->has_idr == _TRUE_){
    ...
    class_store_columntitle(titles,"dmu_idr_1",_TRUE_);
    class_store_columntitle(titles,"dmu_idr_2",_TRUE_);
    //class_store_columntitle(titles,"dmu_idr_3",_TRUE_);
    class_store_columntitle(titles,"tau_idr_1",_TRUE_);
    class_store_columntitle(titles,"tau_idr_2",_TRUE_);
    //class_store_columntitle(titles,"tau_idr_3",_TRUE_);
    class_store_columntitle(titles,"g_idm_dr_1 [Mpc^-1]",_TRUE_);
    class_store_columntitle(titles,"g_idm_dr_2 [Mpc^-1]",_TRUE_);
    //class_store_columntitle(titles,"g_idm_dr_3 [Mpc^-1]",_TRUE_); 
    class_store_columntitle(titles,"sum_g_idm_dr [Mpc^-1]",_TRUE_);   
  }
...
int thermodynamics_output_data(struct background * pba,
  ...
  int index_z, storeidx, n;
  double *dataptr, *pvecthermo, *last_pvecthermo;
  double z,tau, last_z, last_tau, sum_g;

  /* Finding tau for SInu, I am well aware this is not the place to do this, but it works, so ill fix it later -JH*/
  for(index_z = 1; index_z<pth->tt_size; index_z++){
    pvecthermo = pth->thermodynamics_table+index_z*pth->th_size;
    last_pvecthermo = pth->thermodynamics_table+(index_z-1)*pth->th_size;
    z = pth->z_table[index_z];
    last_z = pth->z_table[index_z-1];
    ...
    for(n=0; n<2; n++){
      pvecthermo[pth->index_th_tau_idr+n] = last_pvecthermo[pth->index_th_tau_idr+n] + (pvecthermo[pth->index_th_dmu_idr+n] + last_pvecthermo[pth->index_th_dmu_idr+n])*(last_tau-tau)/2;
    }
  }

  /* Finding g for SInu -JH*/
  pvecthermo = pth->thermodynamics_table;
  pvecthermo[pth->index_th_g_idm_dr+3] = 0.;
  for(index_z = 1; index_z<pth->tt_size; index_z++){
    pvecthermo = pth->thermodynamics_table+index_z*pth->th_size;
    sum_g=0.;
    for(n=0; n<2; n++){
      pvecthermo[pth->index_th_g_idm_dr+n] = pvecthermo[pth->index_th_dmu_idr+n]*exp(-pvecthermo[pth->index_th_tau_idr+n]);
      sum_g += pvecthermo[pth->index_th_g_idm_dr+n];
    }
    pvecthermo[pth->index_th_g_idm_dr+3] = sum_g/2;
  }
  ...
  for (index_z=0; index_z<pth->tt_size; index_z++){
    ...
    //9class_store_double(dataptr,pvecthermo[pth->index_th_exp_m_kappa],_TRUE_,storeidx);
    ...
    if(pba->has_idr == _TRUE_){
      ...
      class_store_double(dataptr,pvecthermo[pth->index_th_dmu_idr+1],_TRUE_,storeidx);
      //class_store_double(dataptr,pvecthermo[pth->index_th_dmu_idr+2],_TRUE_,storeidx);
      class_store_double(dataptr,pvecthermo[pth->index_th_tau_idr],_TRUE_,storeidx);
      class_store_double(dataptr,pvecthermo[pth->index_th_tau_idr+1],_TRUE_,storeidx);
      //class_store_double(dataptr,pvecthermo[pth->index_th_tau_idr+2],_TRUE_,storeidx);
      class_store_double(dataptr,pvecthermo[pth->index_th_g_idm_dr],_TRUE_,storeidx);
      class_store_double(dataptr,pvecthermo[pth->index_th_g_idm_dr+1],_TRUE_,storeidx);
      //class_store_double(dataptr,pvecthermo[pth->index_th_g_idm_dr+2],_TRUE_,storeidx);
      class_store_double(dataptr,pvecthermo[pth->index_th_g_idm_dr+3],_TRUE_,storeidx);
    }
  }
