...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  if (pfzw->required_computation_stage >= cs_nonlinear_pt){
    ...
    class_call(nonlinear_pt_init(&pr,&ba,&th,&pt,&pm,&nlpt),
                      nlpt.error_message,
                      errmsg
                      );
  }
  ...
  if (pfzw->required_computation_stage >= cs_transfer){
    ...
    class_call_except(transfer_init(&pr,&ba,&th,&pt,&nlpt,&nl,&tr),
                      tr.error_message,
                      errmsg,
...
int input_get_guess(double *xguess,
  ...
  struct spectra sp;          /* for output spectra */
  struct nonlinear nl;        /* for non-linear spectra */
  struct nonlinear_pt nlpt;   /* for non-linear_pt spectra*/
  struct lensing le;          /* for lensed spectra */
  struct output op;           /* for output files */
  int i;
